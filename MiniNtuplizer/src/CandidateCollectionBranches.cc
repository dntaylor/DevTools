...
template<typename T>
CollectionFunction<T>::CollectionFunction(TTree * tree, std::string functionName, std::string functionString):
...
template<typename T>
void CollectionFunction<T>::evaluate(const reco::CandidateView& candidates)
{
  values_.clear();
  ...
  branches_(iConfig.getParameter<edm::ParameterSet>("branches"))
{
  ...
  for ( auto functionName : branches_.getParameterNames() ) {
    ...
    if (functionType=='F') {
      floatFunctions_.emplace_back(new CollectionFloatFunction(tree, branchName, functionString));
    }
    else if (functionType=='I') {
      intFunctions_.emplace_back(new CollectionIntFunction(tree, branchName, functionString));
    }
